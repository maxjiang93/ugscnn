...
class Up_intp(nn.Module):
    def __init__(self, level, mesh_folder, bias=True):
        """
        use mesh_file for the mesh of one-level up
        """
        super().__init__()
        mesh_file = os.path.join(mesh_folder, "icosphere_{}.pkl".format(level))
        self.up = MeshIntp(mesh_file)
...
class UNet_intp(nn.Module):
    def __init__(self, mesh_folder, in_ch, out_ch, max_level=5, min_level=0, fdim=16):
        ...
        for i in range(self.levels):
            self.down.append(Down(fdim*(2**i), fdim*(2**(i+1)), max_level-i-1, mesh_folder))
            self.channels.append(fdim*(2**(i+1)))
        # Upward path
        for i in range(self.levels):
            self.up.append(Up_intp(min_level+i+1, mesh_folder))
        self.out_conv = nn.Sequential(nn.Conv1d(sum(self.channels), 200, self.nv_max), 
                                      nn.Conv1d(200, out_ch, self.nv_max))
        self.down = nn.ModuleList(self.down)
        self.up = nn.ModuleList(self.up)
